...
func (uc *UserCreate) AddReportedIDs(ids ...int) *UserCreate {
	uc.mutation.AddReportedIDs(ids...)
	return uc
}

// AddReported adds the "reported" edges to the Report entity.
func (uc *UserCreate) AddReported(r ...*Report) *UserCreate {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
	return uc.AddReportedIDs(ids...)
}
...
func (uc *UserCreate) AddReporter(r ...*Report) *UserCreate {
	ids := make([]int, len(r))
	for i := range r {
		ids[i] = r[i].ID
	}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.ReportedIDs(); len(nodes) > 0 {
